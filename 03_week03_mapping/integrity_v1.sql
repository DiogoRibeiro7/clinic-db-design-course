...
-- 4) Orphan prescription items to medication
SELECT pi.prescription_item_id
FROM prescription_item pi
LEFT JOIN medication m ON m.medication_id = pi.medication_id
WHERE m.medication_id IS NULL;

-- 5) Appointments with missing patient/doctor/room
...
-- 6) Duplicate room codes
SELECT room_code, COUNT(*) AS duplicate_count
FROM room
GROUP BY room_code
HAVING COUNT(*) > 1;

-- 7) Medications with non-positive strength
SELECT medication_id, strength_mg
FROM medication
WHERE strength_mg <= 0;
