...
## Windows (PowerShell)
```powershell
scripts\run_week.ps1 -WeekPath 03_week03_mapping -User root -Database clinic_db -Host 127.0.0.1 -Port 3306
```

## macOS/Linux (bash)
```bash
./scripts/run_week.sh --week_path 03_week03_mapping --user root --database clinic_db --host 127.0.0.1 --port 3306
```
